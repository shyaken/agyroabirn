...
#Region ### START Koda GUI section ### Form=
	...
	$okbutton = GUICtrlCreateButton("Start", 88, 392, 75, 25, 0)
	$outbutton = GUICtrlCreateButton("Quit", 170, 392, 75, 25, 0)
...
While 1
	...
	Select
		...
		Case $msg = $gui_event_close
			MsgBox(0, "Goodbye", "See you later")
			ExitLoop
		Case $msg = $outbutton
			MsgBox(0, "Goodbye", "Have great time")
			ExitLoop
	EndSelect
WEnd
...
Func startSpam()
	While 1
		sleep(1000)
		$randKey = Random(0,100)
		$randDir = Random(0,100)
		Select
			Case $randKey <= 2
				$key = ","
			Case $randKey > 2 And $randKey <= 5
				$key = "a"
			Case $randKey > 5 And $randKey <= 75
				$key = "o"
			Case $randKey > 75
				$key = "e"
		EndSelect
		...
		Select
			Case $randDir <= 25
				$dkey = "{UP}"
			Case $randDir > 25 And $randDir <= 50
				$dkey = "{LEFT}"
			Case $randDir > 50 And $randDir <= 75
				$dkey = "{RIGHT}"
			Case $randDir > 75
				$dkey = "{DOWN}"
		EndSelect
